...
export default function App() {
  ...
  async function loadOrCreateProfile(user) {
    ...
    if (!profile) {
      const { data: newProfile } = await supabase
        .from('profiles')
        .insert({ id: user.id, email: user.email, name: user.email, role: 'client' })
        .select().single()
      profile = newProfile
    }

    // Always make sure your email is trainer
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', fontFamily: "'Jost', sans-serif", fontWeight: 300, fontSize: 14, color: '#D4B896', background: '#F5EDE3' }}>
      Loading...
    </div>
  )
